...
- **Actions do CI pinadas por SHA (Rodada 7)** — `actions/checkout` e `actions/setup-node` nos 2 workflows deixam de referenciar tag móvel (`@v4`/`@v5`) e apontam para commit imutável (40 hex) com comentário da versão — elimina o risco de supply chain de uma tag sobrescrita trocar o código do CI sem mudança no workflow (achado 29). SHAs resolvidos via GitHub API (todos `type: commit`). Checagem estática no `security:check` (item 23) rejeita qualquer `uses: owner/repo@vN` futuro — **32/32 checagens**.
