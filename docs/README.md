...
| [auditoria-seguranca.md](auditoria-seguranca.md) | Auditoria de segurança — 9 rodadas (path traversal, SSRF + trailing dot + DNS wildcard + IPv6 transition, redirects + origin guard, log injection, prototype pollution, OAuth/re-auth, DoS: stack overflow + ciclo/OOM, loop de paginação, espera de rate limit/retry, supply chain: CI permissions, npm audit, deps com versão real, actions por SHA, PKCE multi-instância) — Rodada 9 em [auditoria-pente-fino.md](auditoria-pente-fino.md) |
| [auditoria-pente-fino.md](auditoria-pente-fino.md) | Auditoria Rodada 9 — rastro do pente fino (8 achados A1–A8 corrigidos e testados) |
