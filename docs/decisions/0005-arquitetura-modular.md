...
- **Escolhida:** pacotes por domínio sobre o monorepo npm workspaces (ADR-0001), porque permite instalação seletiva (`@nodemelivre/core` + `@nodemelivre/items`), versionamento independente e testes por pacote com aliases do Vitest apontando para o `src`.
...
- (-) Nove pacotes para manter; mitigado por convenção (`kebab-case`, mesmo `tsconfig.base.json`, mesmo Biome).
- (-) `npm run build` precisa de ordem explícita (core → types → auth → resources → sdk), pois o npm não ordenou por topologia com `--workspaces`.
