...
Uma **rodada de confiabilidade (Rodada 11)** fechou os pontos de processo que a segurança de código não cobre: **supply chain** (package-lock.json em v3 exigido e verificado, `npm ci` obrigatório nos dois workflows, **SBOM CycloneDX gerado no CI** e `npm audit --audit-level=high` explícito — 4 checagens estáticas novas), **endurecimento do `FileTokenStore`** (diretório criado com **0700** e **re-permissionado** se já existir frouxo; token e backup **0600** re-forçados por `chmod` mesmo em arquivo pré-existente — o `mode` do `writeFile` só vale na criação, um backup legado 0644 não seria re-permissionado sem isso) e o **fuzzer do caminho HTTP real** (novo **Estágio 8**: `client-fuzz.test.ts` exercita o `HttpClient.request` inteiro com fetch hostil — cadeias de redirect com `Location` maliciosos/downgrade/lixo, status de retry, retry-after corrompido e bodies malformados; verificou os invariantes de segurança/orçamento já garantidos por mutação). O `security:check` subiu para **76/76** e a suíte para **406 testes**.
...
O `security:check` passou de **36 → 57 checagens** na Rodada 9, para **60** no pente fino de continuidade (B1–B3), para **70/70** na caçada ampla (C1–C10) e para **76/76** na Rodada 11 (confiabilidade):
...
- **Estáticas novas (Rodada 11, 6):** 43 (`package-lock.json` em lockfileVersion 3), 44 (`npm ci` nos workflows), 45 (SBOM CycloneDX no CI), 46 (`npm audit --audit-level` explícito), 47 (fuzzer do HttpClient presente em `client-fuzz.test.ts`) + o Estágio 8 (fuzzer do HttpClient como checagem dinâmica própria).

`npm run security:check` → **76/76** (contrato `--expect-checks 76` no `ci.yml`/`publish-beta.yml` — exit 1 se a contagem mudar).
...
**Estágios próprios no `security:check`:** para que os fuzzers **sempre** rodem no CI de segurança, eles também são estágios próprios do `security:check` (45 → **60/60 checagens**): o fuzzer do `deepOmitEmpty` é o **Estágio 3**, o do `DeduplicatingLogger` o **Estágio 4**, o do `buildUrl` o **Estágio 5**, o do `paginate` o **Estágio 6**, o do `RateLimiter` o **Estágio 7** e o do **`HttpClient` o Estágio 8** (Rodada 11) — execução dedicada com `vitest -t "fuzzing"` que **falha se os testes não rodarem de verdade** (o vitest sai 0 com tudo *skipped* quando o filtro não casa — a checagem parseia `Tests N passed` e exige N ≥ 1) — mais checagens estáticas de presença (itens 36-38, 41-42 e 47) que pegam a remoção/renomeação também no `--static-only`. Todos confirmados por mutação: renomear um describe derruba a checagem estática e o estágio correspondente.

**Reforço Rodada 11 (fuzzing do caminho HTTP real):** os fuzzers 3-7 cobrem utilidades isoladas; o `HttpClient.request` inteiro ficava só com testes unitários — `http/src/client-fuzz.test.ts` (novo **Estágio 8**) exercita o client com um **fetch hostil** que devolve cadeias aleatórias de redirects (`Location` oficiais/maliciosos/downgrade/lixo, incl. `file:`/`javascript:`/`data:`/`//evil.com`/CRLF), status de retry (429/5xx), retry-after corrompido e bodies malformados. Invariantes verificados: **Authorization nunca chega a um origin ≠ do baseUrl** (nem via redirect cross-origin — o token só vale para o origin original); **nunca mais que `MAX_REDIRECTS+1` fetches** (anti-loop); **sem downgrade https→http** (nenhum fetch vai para `http://`); orçamento de retry **exato** (nunca mais que `maxRetries+1` tentativas; erro em 5xx persistente consome exatamente o orçamento); retry-after corrompido **nunca dorme além de `MAX_WAIT_MS`**; bodies hostis **nunca lançam erro nativo de parse** (caem no contrato do SDK); e o **clone do evento response** (O4) que o listener lê não quebra o parse. **Eficácia comprovada por mutação:** as primeiras execuções do fuzzer pegaram 3 erros de invariante no próprio teste (408 não é retryable, `MAX_WAIT_MS` real = 300s, retry do GET idempotente após loop de redirect) — todos corrigidos no fuzzer; o client passou limpo em 400 cadeias + 300 grafos + 200 + 300 + 100 casos.

**Contrato de contagem no CI:** para que uma regressão de cobertura **nunca** passe silenciosamente, `ci.yml` e `publish-beta.yml` rodam `npm run security:check -- --expect-checks 76` — o script falha com **exit 1** se o total de checagens não for exatamente 76 (ABAIXO = alguma checagem foi removida/desativada; ACIMA = contrato desatualizado a atualizar nos workflows e docs). A checagem estática 39 impede que o contrato seja removido dos workflows. Validado por execução: `--expect-checks 76` → exit 0; `--expect-checks 75`/`77` → exit 1.
...
## ✅ Verificações sem problema (reconfirmadas na Rodada 9 + Rodada 10 + Rodada 11)
...
**Rodada 11 (confiabilidade — supply chain, token store, fuzzer HTTP real):**

| Frente | O que foi feito | Status |
|---|---|---|
| Supply chain | `package-lock.json` em v3 exigido/verificado, `npm ci` obrigatório (reproduzível), **SBOM CycloneDX** gerado no CI e `npm audit --audit-level=high` explícito | ✅ Feito (4 checagens estáticas novas) |
| `FileTokenStore` | Diretório **0700** (criado e re-permissionado se frouxo), token e backup **0600** re-forçados por `chmod` mesmo em arquivo pré-existente (o `mode` do `writeFile` só vale na criação) | ✅ Feito (2 testes novos de permissão) |
| Fuzzer HTTP real | Novo **Estágio 8** (`client-fuzz.test.ts`): fetch hostil com cadeias de redirect, status de retry, retry-after corrompido e bodies malformados — 6 invariantes verificados | ✅ Feito (7 testes novos) |
 ...
 *Rodada 11 (confiabilidade) validada com **406 testes** (7 novos do fuzzer HTTP), lint, typecheck, build e `npm run security:check` (**76/76** — 4 checagens de supply chain + fuzzer do HttpClient como Estágio 8 + presença estática; contrato `--expect-checks 76` atualizado nos dois workflows) verdes. Mudanças em `scripts/security-check.mjs`, `packages/auth/src/token.ts` (0700/0600) e novo `packages/http/src/client-fuzz.test.ts`.*
